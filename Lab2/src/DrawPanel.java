...
public class DrawPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if(mode == DrawTypes.LINES) {
            ...
            if (thickness > 1) {
                ...
                var brezX = cursorX;
                var brezY = cursorY;
                BrezLinePreview.line(canvasGraphics2D, color, xPressed, yPressed, brezX, brezY);
            }
            ...
            while(i < corners){
                b = Math.sin( (z + axel)/180*Math.PI);
                a = Math.cos( (z + axel)/180*Math.PI);
                x[i] = cursorX + (int)(a * R);
                y[i] = cursorY - (int)(b * R);
                z = z + angle;
                i++;
            ...
            while(i < corners){
                a1 = Math.cos( (z + axel)/180 * Math.PI);
                b1 = Math.sin( (z + axel)/180 * Math.PI);
                a2 = Math.cos( ((z + axel) + (360 / (2 * corners)))/180 * Math.PI);
                b2 = Math.sin( ((z + axel) + (360 / (2 * corners)))/180 * Math.PI);
            ...
            public void mouseReleased(MouseEvent e) {
                ...
                if (mode == DrawTypes.LINES) {
                    ...
                    if (thickness > 1) {
                        ...
                        BrezLine brezLine = new BrezLine(xSize, ySize);
                        brezLine.line(image, color, xPressed, yPressed, mouseX, mouseY);
                    }
                    repaint();
                } else if (mode == DrawTypes.STAMP) {
                    int x[] = new int [corners];
    ...
    public void stamp() {
        ...
        preferencesFrame.setLocation(1600, 600);
        preferencesFrame.setLayout(null);
        preferencesFrame.setSize(300, 250);
        preferencesFrame.setResizable(false);
        JSlider lineThickness = new JSlider(1, 15);
        lineThickness.setValue(thickness);
        JLabel lineThicknessLabel = new JLabel("Thickness");
        ...
        JSlider axelSlider = new JSlider(0, 360);
        axelSlider.setValue(axel);
        JLabel axelLabel = new JLabel("Rotate");
        axelLabel.setBounds(15, 165, 100, 20);
        axelSlider.setBounds(80, 167, 160, 20);
        ...
        JTextField currentAxel = new JTextField(String.valueOf(axel));
        ...
        currentAxel.addActionListener(e -> {
            if (Character.isLetter(currentAxel.getText().charAt(0))) {
                JOptionPane.showMessageDialog(preferencesFrame, "Letters are NOT NUMBERS!!!");
                currentAxel.setText(String.valueOf(axel));
                return;
            }
            var newValue = Integer.parseInt(currentAxel.getText());
            if (newValue >= 0 && newValue <= 360) {
                axel = newValue;
                axelSlider.setValue(axel);
            } else {
                JOptionPane.showMessageDialog(preferencesFrame, "Corners only >=0 && <=360");
            }
        });
        ...
        axelSlider.addChangeListener(e -> {
            axel = ((JSlider)e.getSource()).getValue();
            currentAxel.setText(String.valueOf(axel));
        });
        ...
        currentAxel.setBounds(250, 165, 30, 20);
        ...
        preferencesFrame.add(axelLabel);
        preferencesFrame.add(axelSlider);
        preferencesFrame.add(currentAxel);
    ...
    public void clearPanel() {
        ...
        imageGraphics2D.fillRect(0, 0, xSize, ySize);
    ...
    public void resizeWorker(int xSize, int ySize) {
