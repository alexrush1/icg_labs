    ...
    public static BufferedImage process(BufferedImage image) {
        ...
        for (int i = 0; i < height - 1; i++) {
            for (int j = 0; j < width - 1; j++) {
                ...
                int newR = currentPixelColor.getRed() * 4 - lowerPixel.getRed() - upperPixel.getRed()
                        - leftPixel.getRed() - rightPixel.getRed();

                int newG = currentPixelColor.getGreen() * 4 - lowerPixel.getGreen() - upperPixel.getGreen()
                        - leftPixel.getGreen() - rightPixel.getGreen();

                int newB = currentPixelColor.getBlue() * 4 - lowerPixel.getBlue() - upperPixel.getBlue()
                        - leftPixel.getBlue() - rightPixel.getBlue();

                //System.out.println("before R: " + newR + " G: " + newG + " B: " + newB);

                newR = clamp(newR, 0, 255);
                if ((double)newR/(double)255 >= 0.05) {
                    newR = 255;
                }
                newG = clamp(newG, 0, 255);
                if ((double)newG/(double)255 >= 0.05) {
                    newG = 255;
                } else { newG = 0; }
                newB = clamp(newB, 0, 255);
                if ((double)newB/(double)255 >= 0.05) {
                    newB = 255;
                } else { newB = 0; }
