...
public class Core {
    ...
    public Core(Preferences preferences, int xSize, int ySize, double xKoef, double yKoef, Graphics2D graphics2D, Board board) {
    ...
    public void span(double min, double max, Color newColor, BufferedImage image) {
        ...
        //System.out.println("zero grid = " + grid[0]);
        for (int x = 0; x <= preferences.N; x++) {
            for (int y = 0; y <= preferences.M; y++) {
                if (grid[y * preferences.N + x] == min) {grid[y * preferences.N + x] = grid[y * preferences.N + x] + 0.0000001;}
                if (grid[y * preferences.N + x] == max) {grid[y * preferences.N + x] = grid[y * preferences.N + x] - 0.0000001;}
